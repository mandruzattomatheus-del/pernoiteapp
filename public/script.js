...
function mostrarFormulario() {
  ...
  if (userFuncaoEl) {
    userFuncaoEl.textContent = getFuncao() === 'motorista' ? 'Motorista' : 'Ajudante';
  }
  ...
  setTimeout(() => {
    ...
    if (selectMotorista) {
      Array.from(selectMotorista.options).forEach(opt => {
        if (opt.text.trim() === nome.trim()) selectMotorista.value = opt.value;
      });
      selectMotorista.classList.remove('is-error');
      selectMotorista.classList.add('is-valid');
    ...
    if (selectAjudante) {
      Array.from(selectAjudante.options).forEach(opt => {
        if (opt.text.trim() === nome.trim()) selectAjudante.value = opt.value;
      });
      selectAjudante.classList.remove('is-error');
      selectAjudante.classList.add('is-valid');
      ...
      if (res.status === 409 && json.duplicado) {
        ...
        await new Promise((resolve) => {
          document.getElementById('modalDuplicidadeConfirmar').onclick = async () => {
            ...
            submitBtn.textContent = 'Enviando...';
            try {
              const res2  = await fetch('/api/registro', {
                method: 'POST',
                headers: getAuthHeaders(),
                body: JSON.stringify({ ...data, confirmar_duplicado: true })
              });
          ...
          document.getElementById('modalDuplicidadeCancelar').onclick = () => {
            modal.style.display = 'none';
            resolve();
          };
        });
  ...
  function mostrarErro(msg) {
    ...
    if (!toast) {
      ...
      toast.style.cssText = `position:fixed; bottom:24px; left:50%; transform:translateX(-50%);
        background:#dc2626; color:#fff; padding:14px 24px; border-radius:4px;
...
(() => {
  ...
  function sortByNameBR(list) {
    return list.slice().sort((a, b) => a.nome.localeCompare(b.nome, 'pt-BR', { sensitivity: 'base' }));
  }

  function fillCities(cities) {
    cityEl.innerHTML = '';
    const first = document.createElement('option');
    first.value = ''; first.textContent = 'Selecionar cidade';
